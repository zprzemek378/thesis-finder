...
const ThesisSidebar = ({ thesis }: ThesisSidebarProps) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 h-full text-left">
      <Link to="/search" className="flex items-center text-[var(--o-blue)] mb-6 hover:underline">
        <svg 
        ...
        <div>
          ...
          <div className="border rounded-md">
            <div className="flex items-center p-3">
                <p>{thesis.supervisor.academicTitle} {thesis.supervisor.firstName} {thesis.supervisor.lastName}</p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="border rounded-md">
            ...
            {Array.from({ length: availableSlots }).map((_, index) => (
              <div key={`empty-${index}`} className="p-4 border-b last:border-b-0 text-[var(--o-green)]">
                Dostępne
              </div>
            ))}
          </div>
