...
router.post('/', verifyAccessTokenMiddleware, async (req: any, res: any, next: NextFunction) => {
    try {
        ...
        if (!Array.isArray(members) || members.length < 2) {
            return res.status(400).json({ message: 'Czat musi mieć co najmniej dwóch członków.' });
        }
        if (!members.some(memberId => memberId.toString() === loggedInUserId.toString())) {
            return res.status(403).json({ message: 'Musisz być członkiem tworzonego czatu.' });
        }
        const validMemberObjectIds: mongoose.Types.ObjectId[] = [];
        for (const id of members) {
            if (!mongoose.Types.ObjectId.isValid(id)) {
                return res.status(400).json({ message: `Nieprawidłowy format ID członka: ${id}.` });
            }
            validMemberObjectIds.push(new mongoose.Types.ObjectId(id));
        }

        const existingUsers = await User.find({ _id: { $in: validMemberObjectIds } });
        if (existingUsers.length !== validMemberObjectIds.length) {
            return res.status(404).json({ message: 'Jeden lub więcej użytkowników nie istnieje.' });
        }

        if (members.length === 2) {
            const sortedMembers = validMemberObjectIds.map(id => id.toString()).sort();
            const existingChat = await ChatModel.findOne({
                members: { $all: sortedMembers, $size: 2 }
            });
            if (existingChat) {
                return res.status(409).json({ message: 'Czat między tymi użytkownikami już istnieje.', chat: existingChat });
            }
        }
        ...
        try {
            ...
            for (const memberId of validMemberObjectIds) {
                await User.findByIdAndUpdate(memberId, { $addToSet: { chats: savedChat._id } }, { session });
            }
...
router.get('/my-conversations', verifyAccessTokenMiddleware, async (req: any, res: any, next: NextFunction) => {
    try {
        const loggedInUserId = req.user!._id;
        ...
        const formattedConversations = chats.map(chat => {
            const otherMember = chat.members.find((member: any) => member._id && member._id.toString() !== loggedInUserId.toString()) as IUser | undefined; 

            const chatName = (otherMember && chat.members.length === 2) ? `${otherMember.firstName} ${otherMember.lastName}` : (chat.title || 'Czat grupowy');
...
router.get('/:chatId/messages', verifyAccessTokenMiddleware, async (req: any, res: any, next: NextFunction) => {
    try {
        ...
        const chat: IChat | null = await ChatModel.findById(chatId).populate('members', 'firstName lastName email');
        if (!chat) {
            return res.status(404).json({ message: 'Nie znaleziono czatu.' });
        }

        const isMember = chat.members.some((member: any) => member._id.toString() === loggedInUserId.toString());
        if (!isMember) {
            return res.status(403).json({ message: 'Nie masz dostępu do tego czatu.' });
        }
